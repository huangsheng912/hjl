...
class Main extends React.Component {
  ...
  render() {
    const {total, tableList, loading, hash, toAddress, amount,transferLoading, inHash, inAddress, inAmount,transferInLoading, page} = this.state;
    const columns = [
      {
        title: '业务',
        key:'transferType',
      ...
      },
      {
        ...
        key:'txHash',
        dataIndex:'txHash',
        render: (text) => <a href={"https://www.etherchain.org/tx/"+text} target='__blank'>{text}</a>
      },
      {
        title: '交易地址',
        key:'address',
        ...
        <div className="transfer-operate">
          <div className="shadow bg-white">
            <div className='content'>
              ...
              <h3>ETH广播交易</h3>
              <div className='item'>
                <TextArea style={{minHeight:'180px'}} value={hash} onChange={this.hashChange} />
              </div>
              <div className='item'>
                <span>交易地址：</span>
                {toAddress}
              ...
              <div className='btn-wrap'>
                <Button type='primary' size='large' onClick={this.broadcast}>广播交易</Button>
              </div>
            </div>
          </div>
        </div>
        <div className="transfer-operate">
          <div className="shadow bg-white">
            <div className='content'>
              ...
              <div className='item'>
                <span>交易hash：</span>
                <Input placeholder='请输入交易hash' value={inHash} onChange={(v)=>this.transferInChange(v,'inHash')} />
              </div>
              <div className='item'>
                <span>转入地址：</span>
                <Input placeholder='请输入转入地址' value={inAddress} onChange={(v)=>this.transferInChange(v,'inAddress')} />
              </div>
              <div className='item' style={{marginBottom: '53px'}}>
                <span>到账数量：</span>
                <Input placeholder='请输入到账数量' value={inAmount} onChange={(v)=>this.transferInChange(v,'inAmount')} />
              </div>
              <div className='btn-wrap'>
                <Button type='primary' size='large' onClick={this.transferIn}>转入</Button>
              </div>
            </div>
          </div>
        </div>
