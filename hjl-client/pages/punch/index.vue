	...
	<view class="punch-page">
		...
		<view class="pay-wrap ">
			<view class="content pd-lr-15">
				...
				<button type="primary" class="fr" @click="submit" :disabled="disbled" v-if="hasLogin">发布</button>
				<operate-button class="fr" type="auth" v-else>发布</operate-button>
			</view>
		</view>
	...
	export default {
		...
		methods: {
			...
			async newWordsOfPlace() {
				...
				if (res.result) {
					if (this.pay && res.result.billInfo) {
