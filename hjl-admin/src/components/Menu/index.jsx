...
class RenderRoute extends React.Component {
  ...
  shouldComponentUpdate(nextProps, nextState, nextContext) {
    ...
    if (this.props.currentPath === nextProps.currentPath || !this.props.currentPath) {
      return false
    }
    return true
  }
  renderRoute = (config) => {
    ...
    return config.map(v => {
      if (!v.children) {
        ...
        return <Route exact path={v.value} key={v.value} component={v.component} />
      } else {
    ...
    return (
      <div className='tab-route'>
        <div className='tab-wrap scrollbar'>
          {tabData.map((v,i)=>{
            return <span key={i} className={v.choose?'active-tab tab-item':'tab-item'} onClick={() => this.clickTab(v)}>
                {v.title}
                {
...
class MenuBar extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    ...
    if (prevProps.location.pathname !== this.props.location.pathname) {
      this.props.configStore.changeRoute(this.props.location.pathname+this.props.location.search);
    }
  }
  ...
  render() {
    const {breadCrumb, selectKey, tabData, configInfo} = this.props.configStore;
