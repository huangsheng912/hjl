...
class Main extends React.Component {
  ...
  render() {
    const {usdi,nuls,list=[],total,minAmount,maxAmount,tableLoading,page} = this.state;
    const coinType=[{label:'全部',value:'All'},{label:'NULS',value:'NULS'},{label:'USDI',value:'USDI'}];
    const statusType=[{label:'全部',value:'All'},{label:'已确认',value:'Confirm'},{label:'未确认',value:'Pending'}];
    const columns=[
      {
        title: '高度',
        key: 'height',
      ...
      <div className='redemption-page'>
        ...
        <div className="bg-white">
          <div className='list-search hidden'>
            <div className='fl'>
              币种&nbsp;&nbsp;&nbsp;
              <Select defaultValue={'All'} onChange={(v)=>this.handleChange(v,'coinType')}>
                {
                  coinType.map((v,i)=><Option value={v.value} key={i}>{v.label}</Option>)
                }
              </Select>
            </div>
            <div className='fl'>
              状态&nbsp;&nbsp;&nbsp;
              <Select defaultValue={'All'} onChange={(v)=>this.handleChange(v,'status')}>
                {
                  statusType.map((v,i)=><Option value={v.value} key={i}>{v.label}</Option>)
                }
              </Select>
            </div>
            <div className='fl'>
              金额范围&nbsp;&nbsp;&nbsp;
              <Input value={minAmount} onChange={(v)=>this.changeMinBalance(v,'minAmount')} />
              &nbsp;&nbsp;-&nbsp;&nbsp;
              <Input value={maxAmount} onChange={(v)=>this.changeMinBalance(v,'maxAmount')}/>
            </div>
            <div className='fr'>
