            ...
            <p className='item-title'>{v.name}</p>
              {
                Array.isArray(v.value)?
                  v.value.map((item,index)=>
                    <p key={index} className='item-value item-balance'><span>{item}</span>{v.unit[index]}</p>
                  ):
...
class Main extends React.Component {
  ...
  render() {
    const {
      ...
      coldWalletUsdtBalance, coldWalletEthBalance, bigRedeemTotal, userCount, smallBalanceUserCount,
      effectiveUserCount, yesterdayAddUserCount, assetTotal, effectiveAssetTotal, yesterUsdiEarning,
      yesterNulsEarning, mortgateRate, mortgateTotal, nulsPrice, agentCount, outTotal, inTotal, offset, liabilities,
      hotAdminBalance
    } = this.state;
    const walletData = [
      { name: '准备金账户余额', value: [hotWalletEthBalance,hotWalletUsdtBalance], unit: ['ETH','USDT']},
      { name: '当前准备金率', value: hotWalletRate},
      { name: '今日快速提现剩余额度', value: todayRedeemBalance, unit: 'USDI'},
      { name: '缺口', value: difference, unit: 'USDT'},
      { name: '冷钱包余额', value: [coldWalletEthBalance,coldWalletUsdtBalance], unit: ['ETH','USDT']},
    ...
    const userAssetsData = [
      ...
      { name: '产生收益账户持币总数', value: effectiveAssetTotal, unit: 'USDI'},
