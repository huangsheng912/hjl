...
class Main extends React.Component {
  ...
  async getInfo(id) {
    ...
    if (res.result) {
      res.result.companyCreateDate = moment(res.result.companyCreateDate,'YYYY-MM-DD')
  ...
  render() {
    ...
    const {pictures, credentials, visible, previewImgs, activeIndex,
      disabled, defaultValue, showAudit,auditType, showEditorError} = this.state
    ...
    const cLabel = (
      <span>工商营业执照&组织结构代码证&nbsp;&nbsp;<br/>&税务登记证</span>
    );
    ...
    return (
      <div className='edit-scenic-page'>
        <Form {...formItemLayout} style={{maxWidth: '700px'}}>
          ...
          <Form.Item label="景区所属省市">
            {getFieldDecorator('province', {
              initialValue: defaultValue.provinceCode?[defaultValue.provinceCode,defaultValue.cityCode]:'',
              rules: [{ type: 'array', required: true, message: '请选择景区所属省市' },],
          ...
          <Form.Item label="景区介绍" className='editor-item'>
            ...
            <div className="ant-form-explain">{showEditorError?'请输入景区介绍':''}</div>
          </Form.Item>
          ...
          <Form.Item label={cLabel} >
            {getFieldDecorator('credentials', {
              rules: [{ required: true, message: '请上传工商营业执照&组织结构代码证&税务登记证' }],
            })(
              <Upload name="credentialsPic"
                      // listType="picture"
                      accept='image/*'
            ...
            <div>
              {
                credentials.map((pic,index)=> <PicWrap src={pic}
                                                       key={pic}
                                                       remove={()=>this.removeImg(pic,'pictures')}
          ...
          <Form.Item {...tailFormItemLayout}>
            {
              disabled?
                <div>
                  <Button type="primary" onClick={()=>this.showModal('pass')} style={{marginRight: '30px'}}>通过</Button>
        ...
        >
          {
            auditType==='pass'? '更新内容将于次日0:00生效' :
              <TextArea value={this.state.rejectReason} onChange={this.handleChange}/>
          }
        </Modal>
