...
class Main extends React.Component {
  ...
  render() {
    const {total, tableList, addressList, addressInfo={},loading,page} = this.state;
    const transferType=[{label:'全部',value:'All'},{label:'转入',value:'转入'},{label:'转出',value:'转出'}]
    const columns = [
      {
        title:'资产',
        key:'assets',
    ...
    return (
      <div className='transfer-out'>
        <div className='transfer-operate'>
          <div className='shadow bg-white'>
            ...
            <div className="payment-address mb_20">
              <p className='name '>收款地址&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                <Button type='primary' size={"small"} onClick={()=>this.modal.visible()}>选择</Button>
              </p>
              {
                addressInfo.address?
            ...
            <div className='transfer-amount mb_20'>
              <p className='name'>转账金额<span><span className='gray fr'>可转出金额：273.39</span></span></p>
              <Input placeholder='最小提现数量0.1'/>
            </div>
            <div className='transfer-fee mb_20 hidden'>
              <p className='name fl'>手续费&nbsp;&nbsp;&nbsp;<span className='gray'>0.001USDT</span></p>
              <p className='name fr'>到账数量&nbsp;&nbsp;&nbsp;<span className='gray'>223.93 USDT</span></p>
            </div>
            <div className='tc'><Button type='primary' size='large'>转出</Button></div>
          </div>
        </div>
        <div className='transfer-list bg-white'>
          <Select defaultValue='All' onChange={this.handleChange}>
            {
              transferType.map((v,i)=><Option value={v.value} key={i}>{v.label}</Option>)
            }
          </Select>
          <Table
            columns={columns}
        ...
        <AddressModal ref={(v)=>this.modal=v} data={addressList} selectAddress={this.selectAddress}/>
      </div>
    )
  }
}
